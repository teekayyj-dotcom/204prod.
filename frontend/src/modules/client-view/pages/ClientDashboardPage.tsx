    ...
    useEffect(() => {
        ...
        Promise.all(promises)
            .then(([projects, billingSummary]) => {
                ...
                // Compute pending demos count and build action items based on real data
                let demosCount = 0;
                const dynamicActions: ActionItem[] = [];

                clientProjects.forEach((proj) => {
                    let hasPendingDemo = false;
                    ...
                    if (hasPendingDemo) {
                        demosCount++;
                        dynamicActions.push({
                            id: `act-review-${proj.slug}`,
                            title: `Xem & Duyệt Bản dựng: ${proj.title}`,
                            desc: `Dự án "${proj.title}" đang chờ bạn cho ý kiến phản hồi hoặc phê duyệt thiết kế.`,
                            type: "review",
                            link: `/client/projects/${proj.slug}/playback${videoUrl ? `?video=${encodeURIComponent(videoUrl)}` : ''}`,
                ...
                if (billingSummary && billingSummary.pending > 0) {
                    setUnpaidAmount(billingSummary.pending);
                    dynamicActions.push({
                        id: "act-billing",
                        title: "Thanh toán hóa đơn đến hạn",
                        desc: `Bạn có dư nợ chưa thanh toán (${billingSummary.pending.toLocaleString("vi-VN")} đ). Vui lòng thanh toán để tránh gián đoạn tiến độ.`,
                        type: "billing",
                        link: "/client/billing",
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-5">
                    ...
                    <div className="space-y-4">
                        {actions.length === 0 ? (
                            <div className="p-6 text-center border border-[#2E2020]/60 bg-[#1D1616]/40 rounded-xl text-gray-400 text-sm">
                                Hiện tại không có yêu cầu hành động gấp nào.
                            </div>
                        ) : (
                            actions.map((act) => (
                                <div
