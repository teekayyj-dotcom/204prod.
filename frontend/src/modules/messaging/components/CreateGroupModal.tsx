...
export function CreateGroupModal({ onClose, onSuccess }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem("token") || "";
      const conv = await messagingApi.createConversation(true, Array.from(selectedIds), name.trim(), token);
      onSuccess(conv.id);
    } catch (err: any) {
    ...
    <div className="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
        <div className="p-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
          <h2 className="font-semibold text-slate-800">Create New Group</h2>
        ...
        <div className="p-4 overflow-y-auto flex-1">
          ...
          <div className="border border-slate-200 rounded-lg overflow-hidden flex-1 min-h-[200px]">
            {loading ? (
